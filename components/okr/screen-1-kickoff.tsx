...
export function Screen1Kickoff() {
  ...
  useEffect(() => {
    ...
    const loadExistingOKRs = async () => {
      if (authLoading || !supabaseUser || hasLoadedExistingData) {
        return
      }
      ...
      try {
        const timeoutPromise = new Promise<{ data: null; error: { message: string } }>((resolve) => {
          setTimeout(() => {
            resolve({ data: null, error: { message: 'Query timeout - RLS may be blocking' } })
          }, 10000)
        })
        
        const queryPromise = supabase
          .from('okr_generations')
          .select('id, user_id, website_url, company_name, planning_period, strategic_narrative, okrs, is_active, created_at')
        ...
        const okrRecord = Array.isArray(okrData) ? okrData[0] : okrData
        ...
        if (okrData2.okrs && Array.isArray(okrData2.okrs)) {
          const objectives = transformToObjectives(okrData2.okrs as LamaticObjective[])
          setLeaderOKRs(objectives)
        } else if (typeof okrData2.okrs === 'string') {
          const parsed = JSON.parse(okrData2.okrs)
          const objectives = transformToObjectives(parsed)
          setLeaderOKRs(objectives)
        }

        if (okrData2.website_url) {
          const domain = okrData2.website_url.replace(/^https?:\/\//, "").replace(/\/$/, "")
          setCompanyProfile({
            domain,
            name: okrData2.company_name || "",
            industry: "",
            size: "",
            publicNotes: "",
          })
        ...
        setPlanningPeriod(okrData2.planning_period || "")
        if (okrData2.strategic_narrative) {
          setShareableSummary({
            title: `${okrData2.planning_period || 'OKR'} Strategic Priorities — ${userData?.role || 'Leader'}, ${okrData2.company_name || ''}`,
            text: okrData2.strategic_narrative,
        ...
        if (userData || supabaseUser) {
          setCurrentUser({
            ...
            role: userData?.role || "",
            companyDomain: okrData2.website_url?.replace(/^https?:\/\//, "").replace(/\/$/, "") || "",
          })
        }
  ...
  const handleGenerate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const okrData = await generateLeaderOKRs({
        ...
        website_data: companyData.website_data || "",
      })
      ...
      const finalEmail = supabaseUser.email!
    ...
    try {
      ...
      if (planningPeriod) {
        ...
        saveOKRsToDatabase(savePayload)
          ...
          .catch((saveError) => {
            console.error('Failed to save regenerated OKRs:', saveError)
          })
      }
    } catch (error) {
